...
export default function ClientSideToolsPage() {
  ...
  const { messages, sendMessage, status, error, stop, addToolResult } =
    useChat<ChatMessage>({
      ...
      // tells the hook to automatically send the message when the last assistant message is complete with tool calls
      sendAutomaticallyWhen: lastAssistantMessageIsCompleteWithToolCalls,
      async onToolCall({ toolCall }) {
        if (toolCall.dynamic) {
          // dynamic tools are tools which input and output are not known in advance
          return;
        }
